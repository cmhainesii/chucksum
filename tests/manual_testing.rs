...
use chucksum::save_file::ItemStorage;


#[test]
fn manual_testing() -> std::io::Result<()> {
    run_manual_testing()
}

fn run_manual_testing() -> std::io::Result<()> {
    ...
    //     }
    // }
    match save_file.copy_party_pokemon(3, 3) {
        Ok(()) => (),
        Err(e) => println!("Error: {e}")
    }

    if let Err(e) = save_file.copy_party_pokemon(3, 9) {
        println!("Error: {e}");
    }

    if let Err(e) = save_file.copy_party_pokemon(3, 12) {
        println!("Error: {e}");
    }
    ...
    // match save_file.get_box_pokemon_data(3) {
    //     Ok(pokemon_list) => print_pokemon_list(pokemon_list),
    //     Err(e) => println!("Error: {e}"),
    // }
